/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

package pila.jvm.instrucciones;

import pila.jvm.Constantes;

/**
 * result = value1 / value2
 * ..., T value1, T value2 -> ..., T result
 */
public class Tdiv extends InstJvm {

    public Tdiv(byte tipo) throws Exception {
        switch(tipo) {
            case Constantes.T_INT:
                opcode = Constantes.IDIV;
                break;
            case Constantes.T_LONG:
                opcode = Constantes.LDIV;
                break;
            case Constantes.T_FLOAT:
                opcode = Constantes.FDIV;
                break;
            case Constantes.T_DOUBLE:
                opcode = Constantes.DDIV;
                break;
            default:
                throw new Exception("Tipo "+tipo+" inválido");
        }
    }
}
